  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Could not fetch badges, using empty array:", badgesError);
        setBadges([]);
      }
      // Try to fetch activities (warnings) from API
      try {
        const activityResponse = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/api/users/activity`,
          config
        );
          ...
          <div className="bg-gray-800 rounded-xl shadow-lg p-8 border border-gray-700">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-2xl font-bold flex items-center gap-3 text-indigo-400">
                <FiTrendingUp className="text-indigo-400" /> Warnings & Reports
              </h3>
            </div>
            ...
            {activities.length > 0 ? (
              <div className="space-y-6">
                {activities.map((activity) => (
                  <div
                    key={activity._id}
                    className="bg-gradient-to-br from-yellow-500/5 to-yellow-400/5 backdrop-blur-sm border border-yellow-500/20 rounded-xl p-6 shadow-sm hover:shadow-lg transition duration-300"
                  >
                    {/* Header */}
                    <div className="flex justify-between items-center mb-4">
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-yellow-400/20 text-yellow-300 uppercase tracking-wider">
                        {activity.type}
                      </span>
                    ...
                    {activity.relatedContent && (
                      <div className="bg-gray-700/50 rounded-lg p-4 border border-gray-600">
                        <p className="text-xs uppercase text-gray-400 mb-2 tracking-wide font-semibold">
                          Reported Content
                        </p>
                        <p className="text-gray-100 text-base font-medium break-words">
                          "{activity.relatedContent}"
                        </p>
                      </div>
                    )}
              ...
              <div className="flex flex-col items-center justify-center text-center py-16">
                <div className="bg-gray-700 p-6 rounded-full mb-6 shadow-inner">
                ...
                <p className="text-gray-400 text-sm max-w-md">
                  You haven't received any warnings yet. Keep following the
